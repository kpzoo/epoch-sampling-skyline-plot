...
addpath(genpath('/Users/kp10/Documents/MATLAB')); % CHANGE
...
%% Process sampled-coalescent trees

% Possible trajectories to select
trajNames = {'logis', 'exp', 'steep', 'unif_low', 'unif_high', 'boom', 'cyc', 'bottle', 'mesa'};
ntraj = length(trajNames);

% Folder to load from for each trajectory
folload = cell(1, ntraj);
for i = 1:ntraj
    folload{i} = 'test'; % CHANGE based on where data is generated
end

% Folder to save data, deps on test
savetrue = 0;
folsave = 'test20_new';
...
%     ha(2).FaceAlpha = 1; ha(2).FaceColor = 'c';
%     hb.Color = [1 1 1]; hc.Color = [1 1 1];
    stairs(tLin, betaTrue0, '-.', 'color', 'k', 'linewidth', 2);
    hold on;
    stairs(tLin, betaEst0, '-', 'color', 'c', 'linewidth', 2);
    hold off; grid off; box off;
